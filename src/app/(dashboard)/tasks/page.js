...
import {
  ...
  MoreHorizontal, MessageSquare, Send, Trash2, Edit3, ArrowUpRight, X
} from 'lucide-react';
...
export default function TasksPage() {
  ...
  // Form state
  const [form, setForm] = useState({ title: '', description: '', priority: 'medium', expectedCompletionTime: '' });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowCreate(false);
      setForm({ title: '', description: '', priority: 'medium', expectedCompletionTime: '' });
  ...
  const removeCheckInTask = (index) => {
    if (checkInTasks.length > 1) {
      setCheckInTasks(checkInTasks.filter((_, i) => i !== index));
    }
  };
    ...
    <div className="space-y-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-xs text-muted-foreground">
                          ...
                          {task.expectedCompletionTime && (
                            <span>ETA: {task.expectedCompletionTime}</span>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-1">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {!isAdmin && task.status !== 'pending-approval' && task.status !== 'approved' && (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input value={form.expectedCompletionTime} onChange={e => setForm({ ...form, expectedCompletionTime: e.target.value })} placeholder="e.g. 2 hours" />
              </div>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={!!editTask} onOpenChange={() => setEditTask(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => updateMutation.mutate({ id: editTask._id, title: editTask.title, description: editTask.description, priority: editTask.priority, status: editTask.status })}
      ...
      <Dialog open={showCheckIn} onOpenChange={() => {}}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto" onPointerDownOutside={e => e.preventDefault()}>
          <DialogHeader>
            <DialogTitle>Good Morning! Enter Today&apos;s Tasks</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button onClick={submitCheckIn} disabled={dailyMutation.isPending} className="w-full">
      ...
      <Dialog open={!!commentTask} onOpenChange={() => setCommentTask(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-sm">Comments — {commentTask?.title}</DialogTitle>
          </DialogHeader>
          ...
          <div className="flex gap-2">
            <Input value={comment} onChange={e => setComment(e.target.value)} placeholder="Write a comment..." className="flex-1" onKeyDown={e => {
              if (e.key === 'Enter' && comment.trim()) {
                commentMutation.mutate({ id: commentTask._id, content: comment });
              }
            }} />
